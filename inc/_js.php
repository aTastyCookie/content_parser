...
if(isset($_POST['url'])){
		...
		foreach($html->find('script') as $element){
			if(($element->type == "text/javascript" || strpos($element->src,".js")) &&  strlen($element->src)!=''){
				$str['js']['src'][$i] = $element->src;
				if($demo == true && $i>= 5){
						break;
				}
				$i++;
...
<form class="form-horizontal" role="form" method="post" action="">
  ...
  <?php $get= 'js'; require_once('inc/_form_footer.php');?>
</form>